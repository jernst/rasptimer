...
function writeCrontab( $data ) {
    ...
    $script = $dir . "/cron-run.php";

    $open_basedir = "$dir:/tmp/:/usr/share/:/var/log/rasptimer.log";
...
END;
    foreach( $devices as $deviceName => $devicePin ) {
        ...
        if( isset( $p['timeOn'] )) {
            ...
            $file .= "$minOn $hourOn * * * /usr/bin/php -d open_basedir=$open_basedir $script $devicePin 1\n";
            $file .= "$minOff $hourOff * * * /usr/bin/php -d open_basedir=$open_basedir $script $devicePin 0\n";
        }
    } 
